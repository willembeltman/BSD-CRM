...
public class ControllerMethodList
{
    ...
    public ControllerMethodList(NamespacesList namespaceList)
    {
    ...
    public NamespacesList NamespaceList { get; }
    ...
    string CsToTs(string typeName, out bool isNulleble, out bool isList, out bool import, out ModelNamespace? importNamespace)
    {
        ...
        if (tstype == null)
        {
            foreach (var namespaceItem in NamespaceList.ModelNamespaces)
            {
        ...
        if (tstype == null) throw new Exception($"Cannot find TypeScript type for {typeName}");
